...
- Use `x-nodoc-operation-context` on operations when downstream developers need structured guidance about canonical route aliases, workflow/surface placement, header/auth-profile binding, permissions, request-shape variants, pagination, troubleshooting, conditional availability, or provenance. `pagination` supports `none`, `offset`, `cursor`, `link`, `odata`, and `custom` styles plus request parameters, response fields, and notes.
...
- Give a new agent the copy-paste [PORTAL_DISCOVERY_AGENT_PROMPT.md](./PORTAL_DISCOVERY_AGENT_PROMPT.md), which points it to the bounded [AGENT_DISCOVERY_RUNBOOK.md](./AGENT_DISCOVERY_RUNBOOK.md) and the required authenticated CDP setup.
- Use `npm run discover:portal -- --portal <spec-id> --profile bounded --phase plan --json` to generate the exact portal brief, then rerun it with `--profile bounded --phase all --artifacts <artifact-dir>` for deterministic capture, normalized family diffing, and candidate analysis.
- The `all` and `analyze` phases emit a tenant-safe `candidate-handoff.json` that separates review-ready in-scope confirmed GETs, confirmed non-GET or ambiguous safety triage, successful probes, bundle-only validation targets, suppressions, and adjacent scope-review evidence. Adjacent confirmed, probed, and bundle-only candidates retain sanitized host-family routing context but always require explicit specification assignment before review; they are never promotion-active. The evidence-driven recommendation supersedes the plan-time metadata hint; `normalized-family-diff` is not a separate post-run command.
- For Entra and Intune work, run `npm run sync:graph-contract -- --output-dir $graphContract` once per discovery series. `all` and `analyze` verify the pinned cache at `%LOCALAPPDATA%\nodoc-cdp\graph-contract` by default and emit `graph-telemetry.json` plus a bounded `graph-research-queue.json`, including direct operations, expanded `/$batch` members, contract deltas, errors, and documented shape enrichment. Documented Graph calls enrich portal workflow evidence without being copied into portal-owned specs; official-contract deltas stay in a separate Graph research/ownership lane. Raw Graph bodies remain local tenant-sensitive evidence.
- Discovery execution is complete when the run, queue, and handoff artifacts are generated. Specification changes belong in a separate human-reviewed PR and are never applied automatically by discovery.
...
- After capturing a portal pass, run `npm run generate:crawl-candidates -- --spec <title-or-spec-id> --artifacts <artifact-dir>` to normalize the captured route families, scope active candidates to the target spec's hosts and path prefixes, diff them against the checked-in spec, and emit confirmed, probed, or bundle-discovered candidates plus a tenant-safe adjacent scope-review category. Prefer the `discover:portal` `all`/`analyze` workflow when you also need the sanitized promotion/triage handoff and next-action recommendation.
...
- Cross-product shell, support, telemetry, and neighboring portal traffic outside the target spec's host and path scope is retained automatically in the queue and handoff scope-review categories. The legacy `--include-adjacent` flag is accepted for compatibility but does not merge that evidence into active candidates.
