...
- Use `npm run discover:portal -- --portal <spec-id> --profile bounded --phase plan --json` to generate the exact portal brief, then rerun it with `--profile bounded --phase all --artifacts <artifact-dir>` for deterministic capture and candidate analysis.
